try{
    ...
    for(i = 0; i < arr.length; i++){
        if(arr[i].match(/[a-zA-Z0-9]{9}[a-zA-Z0-9-]{3}[0-9]{5}/g) !== null){
            data.push({odo: arr[i-2].replace(/\,/g,''), vin: arr[i]})
        }
    }
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        for (var j = 0; j < element.childNodes.length; j++) {
            ...
            if (node.nodeType === 3) {
                ...
                var replacedText = text.replace(vin, vin + '\nTrue Value™ $' + formatNumber(price.toFixed(0)));
...
async function createOutput(data){
    ...
    for(i = 0; i < data.length; i++){
        const price = await getTrueValue(data[i].vin, data[i].odo).catch(e => null)
        injectPrice(data[i].vin, price)
        output = output + data[i].vin + ': $' + JSON.stringify(price) + '<br>' 
