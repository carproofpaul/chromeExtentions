console.log('content')

//Runs on inital load of buy.adesa.ca
window.addEventListener("load", function load(event){
    setTimeout(() => getText(1), 1000)
}, false);

//Runs when updating current page (e.g. changing 'vehicle per page')
window.addEventListener('popstate', function load(event){
    setTimeout(() => getText(1), 1000)
}, false);


async function getText(count){
    console.log('getting text')
    const text = document.body.innerText
    const arr = text.split(/\s+/)

    let data = []

    for(i = 0; i < arr.length; i++){
        if(arr[i].match(/[a-zA-Z0-9]{9}[a-zA-Z0-9-]{3}[0-9]{5}/g) !== null){
            let odometer = arr[i-2].replace(/\,/g,'');
            if(isNaN(odometer)) odometer = arr[i-3].replace(/\,/g,''); //color was two words
            data.push({odo: odometer, vin: arr[i]})
        }
    }

    if(data.length > 0){
        for(i = 0; i < data.length; i++){
            const vehicle = await getTrueValue(data[i].vin, data[i].odo).catch(e => null)
            if(vehicle !== null) injectPrice(vehicle.vin, vehicle.price)
        }
    } else {
        //wait 1 second before callling again, max 10 times
        //this is if the page loads slowly.
        if(count === 10){
            console.log('nothing found')
            return
        } else {
            console.log('trying again... ' + count)
            setTimeout(() => getText(++count), 1000)
        }
    }
}

function injectPrice(vin, price){
    var elements = document.getElementsByTagName('*');

    for (var i = 0; i < elements.length; i++) {
        var element = elements[i];
    
        for (var j = 0; j < element.childNodes.length; j++) {
            var node = element.childNodes[j];
    
            if (node.nodeType === 3) {
                let text = node.nodeValue;
                if (text === vin) {
                    var br = document.createElement("br");
                    element.appendChild(br)
                    element.appendChild(document.createTextNode('True Value™ $' + formatNumber(price.toFixed(0))), node);
                }
            }
        }
    }
}

const formatNumber = (x) => {
    return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}

function getTrueValue(vin, odo){
    return new Promise((resolve, reject) => {
        const x = ''

        var xmlhttp = new XMLHttpRequest();    
        xmlhttp.open("GET", 'https://devapivaluationwebservice.carproof.com/MlValuation/Retail?request.vin=' + vin + 
        '&request.odometer=' + odo + '&request.fsa=n1g', true);

        xmlhttp.setRequestHeader("webServiceToken", x);
        xmlhttp.send();
    
        xmlhttp.onreadystatechange = (function () {
            if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
                json = JSON.parse(xmlhttp.responseText)
                resolve({price: json.Prediction.Prediction, vin: vin})
            } else if(xmlhttp.readyState === 4 && xmlhttp.status !== 200){
                reject(xmlhttp.status)
            }
        }).bind(this)
    })
}
